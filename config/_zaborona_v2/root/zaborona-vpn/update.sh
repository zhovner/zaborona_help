...
curl -f --fail-early --compressed -o $WORKFOLDERNAME/$FILENAMEDOMAINS "$LISTLINK" || exit 1
curl -f --fail-early --compressed -o $WORKFOLDERNAME/$FILENAMEDOMAINS_CUSTOM "$LISTLINK_CUSTOM" || exit 1
...
LISTSIZE="$(curl -sI "$LISTLINK_CUSTOM" | awk 'BEGIN {IGNORECASE=1;} /content-length/ {sub(/[ \t\r\n]+$/, "", $2); print $2}')"
