...
do
    ...
    echo "/ip route add dst-address=$line gateway=ovpn-out1" >> ./mikrotik-add-ranges_gen.txt
    echo "/ip route remove [find dst-address=$line gateway=ovpn-out1]" >> ./mikrotik-del-ranges_gen.txt
	# Массовое удаление (осторожно)
    #echo "/ip route remove [find dst-address=$line]" >> ./mikrotik-del-ranges.txt
	# Массовое удаление (осторожно)
	#echo "/ip route remove [find gateway=ovpn-out1]" >> ./mikrotik-del-ranges.txt
done < ./ipsdb0.txt
